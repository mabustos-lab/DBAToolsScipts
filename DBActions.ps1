...
function UpdateDBArtifact{
    ...
    if ($latest.count -eq 1) {
         Write-Host "Leyendo paquete de actualización de $targetDBname..."
        ...
        Write-Host "No se encontró ningin paquete de actualización de $targetDBname..."
    }
}
...
switch ($action)
{
    ...
    Default {
        Write-Host "comando de acción no válida '$action'"
    }
}
Stop-Transcript
 Sleep-Progress 5
